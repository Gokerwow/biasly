...
export async function GET(request: Request) {
  ...
  try {
    const response = await fetch(
      "https://kpop.fandom.com/api.php?action=query&list=categorymembers&cmtitle=Category:male%20groups&cmlimit=500&format=json&origin=*"
    );
    ...
    const existingPageIds = new Set(
      existingGroups.map((group) => group.page_id)
    );
    ...
    const { error } = await supabase.from("groups").insert(dataToInsert);
    ...
    return NextResponse.json(
      { error: "Seeding process failed.", details: error.message },
      { status: 500 }
    );
  }
}
