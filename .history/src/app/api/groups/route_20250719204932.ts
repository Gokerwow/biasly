...
export async function GetGroupImage(groupName: string) {
    ...
    if (pageID === "-1" || !Imagedata.query.pages[pageID].thumbnail) {
        return null; // Grup tidak ditemukan di Fandom atau tidak ada gambar
    }
    return Imagedata.query.pages[pageID]
}
...
export async function GET() {
    ...
    try {
        ...
        allGroups.map(async (group) => {
                        const photoUrl = await getFandomPhotoUrl(group.name);
            return {
                ...group, // Salin semua data asli dari RapidAPI
                picture: photoUrl || 'https://via.placeholder.com/150', // Tambahkan foto, atau gambar placeholder jika tidak ditemukan
            };
        })
