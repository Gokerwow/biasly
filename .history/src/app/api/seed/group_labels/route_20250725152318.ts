        ...
        try {
            ...
            if (labelContainer.length > 0) {
                ...
                if (links.length > 0) {
                    ...
                    const containerHtml = labelContainer.html(); // Ambil sebagai HTML, bukan teks
                    if (containerHtml) {
                        const potentialLabels = containerHtml.split('<br>'); // Pisahkan berdasarkan tag <br>

                        potentialLabels.forEach(labelHtml => {
                            // Bersihkan setiap bagian dari tag HTML lain (seperti <sup>) dan spasi
                            const cleanLabel = cheerio.load(labelHtml).root().text().trim();

                            if (cleanLabel) {
                                // Hapus juga tanda kutip jika ada
                                labelNames.add(cleanLabel.replace(/"/g, ''));
                            }
                        });
                    }
                }
            ...
            if (existedLabelsSelectError) {
                throw new Error(`Failed to select labels for ${group.name}: ${existedLabelsSelectError.message}`);
            }
