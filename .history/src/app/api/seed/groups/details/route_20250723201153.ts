    ...
    try {
        ...
        const results = await Promise.all(GroupsData.map(async (group) => {
            try {
                ...
                details.genre = genres.split(',').map(genre => genre.trim())
                ...
                // Fixed: Changed 'genre' to 'genres' and 'details_fetched_at' to 'detail_fetched_at'
                const { error } = await supabase
                    ...
                    .update({
                        ...
                        genre: details.genre, // Changed from 'genre' to 'genres'
                        years_active: details.yearsActive,
                        origin: details.origin,
                        image_url: details.imageUrl,
                        details_fetched_at: new Date().toISOString() // Changed from 'details_fetched_at'
                    })
                    .eq('name', group.name)
