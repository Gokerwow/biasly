    ...
    try {
        ...
        if (!GroupsData || GroupsData.length === 0) {
            return NextResponse.json({
                message: "No groups found with details_fetched_at = null",
                groupsFound: 0
            });
        }
            ...
            try {
                ...
                const { data: updateData, error } = await supabase
                    ...
                    .update({
                        ...
                        genres: details.genres, // Now properly supports text[] array
                        years_active: details.yearsActive, // Now character varying
                        origin: details.origin,
                        image_url: details.imageUrl,
                        details_fetched_at: new Date().toISOString() // Now timestamp without time zone
                    })
                    .eq('name', group.name)
                    .select();
