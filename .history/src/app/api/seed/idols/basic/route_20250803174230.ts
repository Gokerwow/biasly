    ...
    try {
        ...
        for (const category of categories) {
            for (const gender of genders) {
                ...
                while (hasMore) {
                    const url: string = `https://kpop.fandom.com/api.php?action=query&list=categorymembers&cmtitle=Category:${encodeURIComponent(`${encodeURIComponent(gender)} ${encodeURIComponent(category)}`)}&cmlimit=500&format=json&origin=*&cmcontinue=${encodeURIComponent(continueToken ?? '')}`
        ...
        const dataToInsert = IdolArray.map(idolName => ({
            name: idolName
        }));
