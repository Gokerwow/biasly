...
export async function GET() {
    try {
        ...
        // 1. Mengambil semua data dari Fandom API (kode Anda sudah benar)
        ...
        // 2. Mempersiapkan data unik untuk dimasukkan
        ...
        // 3. Melakukan UPSERT DALAM BATCH
        const chunkSize = 500; // Kita akan proses 500 data per batch
        for (let i = 0; i < dataToInsert.length; i += chunkSize) {
            ...
            const { error: insertError } = await supabaseAdmin
                .from('idols') // Pastikan nama tabel Anda 'idol' bukan 'idols'
                .upsert(chunk, { onConflict: 'name' });

            if (insertError) {
                // Jika ada error di salah satu batch, hentikan dan laporkan
