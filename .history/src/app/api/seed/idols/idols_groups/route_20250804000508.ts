    ...
    try {
        ...
        const idolPromises = idolArray.map(async (Idol) => {
            try {
                ...
                const existingGroup = selectGroups?.map(group => group.name) // ngambil namanya ajah
                const unavailableGroups = groups.filter(group => !existingGroup?.includes(group))

                if (unavailableGroups.length > 0) {
                    const groupInsert = unavailableGroups.map(name => {name})
                    const { error: insertError } = await supabaseAdmin
                        .from('groups')
                        .insert('name', unavailableGroups)
                        .select('id, name')
                }
