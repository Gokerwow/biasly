...
export async function GetIdols(gender: string, category: string) {
    try {
        ...
        while (hasMore) {
            const url = `https://kpop.fandom.com/api.php?action=query&list=categorymembers&cmtitle=Category:${encodeURIComponent(categoryName)}&cmlimit=500&format=json&origin=*&cmcontinue=${encodeURIComponent(continueToken ?? '')}`
            ...
            IdolName = result.query.categorymembers.map((name: CategoryMember) => name.title)
        }
