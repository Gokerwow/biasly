...
export async function GetIdolsDetails(idolName: string) {
    try {
        ...
        const occupation = $('h3.pi-data-label:contains("Occupation")')
            .next('.pi-data-value')
            .text()
        detail.occupation = occupation.split(',')

        // Agency
        const Agency = $('h3.pi-data-label:contains("Agency")')
            .next('.pi-data-value')
            .text()
        detail.agency = Agency.split(',')

        // family
        const family = $('h3.pi-data-label:contains("Family")')
            .next('.pi-data-value')
            .text()
        detail.family = family.split(',')

        // fandom color
        const fandom_color = $('h3.pi-data-label:contains("Color(s)")')
            .next('.pi-data-value')
            .text()
        detail.fandom_color = fandom_color.split(',').map((color) => color.trim())
