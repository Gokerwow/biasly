...
export async function GetIdolsDetails(idolName: string) {
    try {
        ...
        const relationship = $('h2.pi-header:contains("Personal Life")')
            ...
            .find('h3')
            .text()
        const relationNameHtml = $('h2.pi-header:contains("Personal Life")')
            .next('div.pi-data')
            .next('.pi-data-value')
            .html()
        const relationNameArrays = relationNameHtml?.split('<br>') //[ el jennie, el krystal] string
        ...
        // const relationName = relationNameArray?.map((item) => )
        const relationPeriode = relationNameArray[1]?.trim() || null
