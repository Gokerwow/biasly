...
export async function GetIdolsDetails(idolName: string) {
    try {
        ...
        const relationName = relationshipText.each((index, Element) => {
            const text = $(Element).text()
            const relationName = $(`h3.pi-data-label:contains('${text}')`)
                .next('.pi-data-value') // el jennie, el krystal
            const relationNameArray = relationName?.each((index, element) => {
                const $item = cheerio.load(element)
                const name = $item.root().find('a').text()
                const periode = $item.root().find('span').text().replace('(', '').replace(')', '')
                return {
                    name: name,
                    periode: periode
                }
        ...
        // const relationNameArray = relationNameArrays?.map((item) => {
        //     const $item = cheerio.load(item)
        //     const name = $item.root().find('a').text() // el jennie => jennie
        //     const periode = $item.root().find('span').text() // el 2018 => 2018
        //     return {
        //         name: name, 
        //         periode: periode
        //     }
        // })
        // [ el jennie, el krystal ]
        // [{name: jennie, periode: 2018}, {name:krystal, periode:2016}]
