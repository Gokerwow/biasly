...
export async function GetIdols(gender: string, category: string) {
    try {
        ...
        const uniqueIdolSet = new Set(IdolArray)
        const uniqueIdolArray  = [...uniqueIdolSet]

        return uniqueIdolArray
...
export async function GET() {
    try {
        ...
        // const idolPromises = allIdolNames.map((idol) => {
        //     return GetIdolsDetails(idol)
        // })
        // const results = await Promise.all(idolPromises)

        return NextResponse.json(allIdolNames)
