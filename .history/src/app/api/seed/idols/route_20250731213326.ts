...
export async function GetIdolsDetails(idolName: string) {
    try {
        ...
        const textInKurung = fullText.match(/\(([^)]*)\)/)?.[0]
        const korean = fullText.match(/Korean:\s*([^;]+)/)?.[1]?.trim() ?? textInKurung?.match(koreanPattern)?.[0]
        const japanese = fullText.match(/Japanese:\s*([^)]+)/)?.[1]?.trim() ?? textInKurung?.match(japanesePattern)?.[0]
