...
export async function GetIdolsDetails(idolName: string) {
    try {
        ...
        const korean = fullText.match(/Korean:\s*([^;]+)/)?.[1]?.trim() ?? textInKurung?.match(koreanPattern)?.[1]
        const japanese = fullText.match(/Japanese:\s*([^)]+)/)?.[1]?.trim() ?? textInKurung?.match(japanesePattern)?.[1]
