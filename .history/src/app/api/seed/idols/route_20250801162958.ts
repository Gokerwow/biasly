...
export async function GET() {
    try {
        ...
        const idolDaraArray = idolData?.map((item) => item.name)
        const idolDataSet = new Set(idolDaraArray)
        ...
        const idolNameClean = new Set(uniqueIdolNameArray.map((item) => item.split(' (')[0].trim()))
        const idolBaru = uniqueIdolNameArray.filter((item) => !idolDataSet.has(item))
