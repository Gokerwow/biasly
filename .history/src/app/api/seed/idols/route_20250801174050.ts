...
export async function GET() {
    try {
        ...
        const normalizedOldData = idolDataDetails?.map((item) => NormalizedDetailFormat(item))
        const normalizedNewData = allIdolDetails?.map((item) => NormalizedDetailFormat(item))
        normalizedNewData.forEach((item) => {
            const existingIdol = normalizedOldData?.find(oldItem => return old.stage_name === newIdol.stage_name;)
        })
