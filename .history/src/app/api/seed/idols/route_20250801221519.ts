...
export async function GET() {
    try {
        ...
        const allIdolNames = idolName.flat() // diratakan jadi satu array yang awalnya arrays of array
        const uniqueIdolNameSet = new Set(allIdolNames) // Dijadiin set biar gada duplikat
        ...
        // Kalo ada idol baru diambil detailnya
        if (idolBaru.length === 0) {
            ...
            console.log(`Ditemukan ${idolBaru.length} idol baru`)
            const detailIdolBaru = [] // nyimpen detail idol baru
            for (const idol of idolBaru) {
                const idolbaru = await GetIdolsDetails(idol)
                detailIdolBaru.push(idolbaru)
                await delay(2000);
            }
            const { error: insertError } = await supabase
                .from('idols')
                .insert(detailIdolBaru)

            if (insertError) {
                console.log(`Gagal insert data idol baru ke db`, insertError.message)
            }
        }

        // Ngecek Detail Idol
        // Ngambil detail dari db
        const { data: idolDataDetails, error: selectErrorDetails } = await supabase
            .from('idols')
            .select('*')

        if (selectErrorDetails) {
            console.log('Error ketika select detail idol dari db', selectErrorDetails.message)
        }
        ...
        for (const item of allIdolDetails) {
            const detailOldIdol = idolDataDetails?.find((oldItem) => oldItem.stage_name === item?.stage_name)
            const normalizedOldDetail = NormalizedDetailFormat(detailOldIdol)
            const normalizedNewDetail = NormalizedDetailFormat(item)
            if (JSON.stringify(normalizedOldDetail) !== JSON.stringify(normalizedNewDetail)) {
                console.log(`data detail untuk idol ${item?.stage_name} berbeda, akan diupdate`)
                const {error: updateError} = await supabase
                    .from('idols')
                    .update(normalizedNewDetail)
                    .eq('stage_name', item?.stage_name)

                if(updateError) {
                    console.log(`Gagal update detail idol ${item?.stage_name}:`, updateError.message);
                }
            }
        }

        return NextResponse.json(`data idol berhasil diambil dan disimpan ke database, total idol baru: ${idolBaru.length}, total detail idol: ${allIdolDetails.length}`);
