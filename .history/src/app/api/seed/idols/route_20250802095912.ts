...
const nextTextNode = bTag[0].nextSibling?.nodeValue ?? ''
const fullText = name + nextTextNode  // contoh: "Ichika (이치카) is a ..."
...
export async function GET() {
    try {
        ...
        if (newIdolNames.length > 0) {
            ...
            for (const name of newIdolNames) {
                const detail = await GetIdolsDetails(name);
                if (detail) newIdolDetails.push(detail);
                await delay(1500); // Delay untuk menghindari rate limit API
            }

            if (newIdolDetails.length > 0) {
                console.log(`Memasukkan ${newIdolDetails.length} idol baru ke database...`);
                const { error: insertError } = await supabase.from('idols').insert(newIdolDetails);
                if (insertError) {
        ...
        if (existingIdolNames.length > 0) {
            ...
            const idolsToUpdate = [];
            for (const name of existingIdolNames) {
                const newDetailApi = await GetIdolsDetails(name);
                if (!newDetailApi) continue; // Lewati jika API gagal memberikan detail

                const oldDetailDb = dbIdolMap.get(name);
                const normalizedNew = NormalizedDetailFormat(newDetailApi);
                ...
                if (JSON.stringify(normalizedNew) !== JSON.stringify(normalizedOld)) {
                    console.log(`Perubahan terdeteksi untuk: ${name}. Akan diupdate.`);
            ...
            if (idolsToUpdate.length > 0) {
                ...
                for (const idol of idolsToUpdate) {
                    const { error: updateError } = await supabase
                        .from('idols')
                        .update(idol)
                        .eq('stage_name', idol.stage_name);

                    if (updateError) {
                        console.error(`Gagal update ${idol.stage_name}:`, updateError.message);
                    }
                }
