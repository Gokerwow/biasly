...
export async function GetIdolsDetails(idolName: string) {
    try {
        ...
        // Group nya
        const groupName = $('h3.pi-data-label:contains("Associations")')
            .next('.pi-data-value')
            .text()
        ...
        if (groupError) {
            console.error(`Gagal mengambil data grup untuk idol ${idolName}:`, groupError.message);
            return null; // Atau bisa throw error jika ingin menghentikan proses
        }
        
        detail.group_id = groupData?.id
