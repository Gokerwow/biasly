    ...
    try {
        ...
        if (newIdolNames.length > 0) {
            ...
            if (newIdolIds.length > 0) {
                const idolsWithGroups = cleanNewIdolDetails.flatMap((idol, index) => {
                    const idolID = newIdolIds[index]
                    const idolsWithGroups = idol?.group_id?.map((group) => { // [group 1, group 2] => [{group_id: 1, idol_id:1}, {group_id: 2, idol_id:2}]
                        return {
                            group_id: group,
                            idol_id: idolID
                        }
