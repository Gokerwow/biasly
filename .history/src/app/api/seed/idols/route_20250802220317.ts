...
export async function GetIdolsDetails(idolName: string) {
    try {
        ...
        const birthDate = $('h3.pi-data-label:contains("Birth date")')
            .next('.pi-data-value')
            .text()
        detail.birth_date = parse(birthDate, 'MMMM d, yyyy', new Date()) ?? null
        ...
        const height = $('h3.pi-data-label:contains("Height")')
            .next('.pi-data-value')
            .text()
        detail.height = Number(height.split(' ')[0]) ?? null

        // weight
        const weight = $('h3.pi-data-label:contains("Weight")')
            .next('.pi-data-value')
            .text()
        detail.weight = Number(weight.split(' ')[0]) ?? null
        ...
        const occupation = $('h3.pi-data-label:contains("Occupation")')
            .next('.pi-data-value')
            .text()
        detail.occupation = occupation.split(',').map((item) => item.trim()) ?? null
        ...
        const fandom_color = $('h3.pi-data-label:contains("Color(s)")')
            .next('.pi-data-value')
            .text()
        detail.fandom_color = fandom_color.split(',').map((item) => item.trim()) ?? null
        ...
        const soloDebutText = $('h3.pi-data-label:contains("Solo debut")')
            .next('.pi-data-value')
            .text()
        detail.solo_debut = parse(soloDebutText, 'MMMM d, yyyy', new Date()) ?? null
...
export async function GET() {
    try {
        ...
        if (newIdolNames.length > 0) {
            ...
            const flatNewIdolDetails = newIdolDetails.flat();
            const setNewidolDetils = new Set(flatNewIdolDetails)
            const newIdolDetailsArray = [...setNewidolDetils]
            const cleanNewIdolDetails = newIdolDetailsArray.filter(detail => detail)
            ...
            if (idolsToInsert.length > 0) {
                console.log(`Memasukkan ${idolsToInsert.length} idol baru ke database...`);
                const { data: insertData, error: insertError } = await supabase
                    .from('idols')
                    .insert(idolsToInsert)
                    .select('id')

                if (insertError) {
                    console.error('Gagal insert idol baru:', insertError.message, newIdolNames);
                    return NextResponse.json({ message: 'Tidak ada idol baru yang valid untuk dimasukkannn.' }, { status: 200 });
                } else {
                    console.log("Berhasil memasukkan idol baru.");
                }

                if (!insertData || insertData.length === 0) {
                    // Ini untuk menangani kasus aneh di mana tidak ada error TAPI juga tidak ada data
            ...
            if (newIdolIds.length > 0) {
                const idolsWithGroups = cleanNewIdolDetails.flatMap((idol, index) => { // kalo map biasa jadi [[{group_id: 1, idol_id:1}, {group_id: 2, idol_id:1}], [{group_id: 1, idol_id:2}, {group_id: 2, idol_id:2}]], pake FlatMap no array of arrays
                    const idolID = newIdolIds[index]
                    const idolsWithGroups = idol?.group_id?.map((group) => { // [group 1, group 2] => [{group_id: 1, idol_id:1}, {group_id: 2, idol_id:1}]
                        return {
                            group_id: group,
                            idol_id: idolID
                        }
                    })
                    return idolsWithGroups
                })

                const { error: groupInsertError } = await supabase
                    .from('idols_groups')
                    .insert(idolsWithGroups)
