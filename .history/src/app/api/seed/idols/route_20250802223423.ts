...
export async function GetIdolsDetails(idolName: string) {
    try {
        ...
        const height = $('h3.pi-data-label:contains("Height")')
            .next('.pi-data-value')
            .text()
        detail.height = Number(height.split(' (')[0].replace(' cm', '').trim()) ?? null

        // weight
        const weight = $('h3.pi-data-label:contains("Weight")')
            .next('.pi-data-value')
            .text()
        detail.weight = Number(weight.split(' (')[0].replace(' cm', '').trim()) ?? null
        ...
        if (newIdolNames.length > 0) {
            ...
            if (idolsToInsert.length > 0) {
                ...
                if (insertError) {
                    ...
                    return NextResponse.json({ message: 'Tidak ada idol baru yang valid untuk dimasukkannn.', newIdolNames }, { status: 200 });
                } else {
