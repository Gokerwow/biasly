...
export async function GET() {
    try {
        ...
        const dbIdolMap = new Map(dbIdols.map(idol => [idol.name, idol]));
        ...
        if (newIdolNames.length > 0) {
            ...
            if (idolsToInsert.length > 0) {
                ...
                if (insertError) {
                    ...
                    return NextResponse.json({ message: 'Tidak ada idol baru yang valid untuk dimasukkannn.', newIdolNames, existingIdolNames, failedNames }, { status: 200 });
                } else {
        ...
        return NextResponse.json({
            ...
            dbIdolMap: dbIdolMap,
            uniqueApiIdolNames: uniqueApiIdolNames
        });
