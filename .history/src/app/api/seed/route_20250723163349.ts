    ...
    GroupsData.forEach(async (group) => {
        ...
        if (debutValueDiv.length > 0) {
            ...
            if (debutHtml) {
                ...
                // Loop untuk setiap entri debut (Korea, Japan, dll.)
                for (const entryHtml of entries) {
                    // ✅ Perbaikan Bug: Cek 'entryHtml', bukan 'debutHtml'
                    if (entryHtml.trim() === '') continue; // Lewati jika ada baris kosong

                    // Muat setiap potongan HTML ke cheerio agar bisa di-parse
                    const $entry = cheerio.load(entryHtml);

                    // Ambil negara dari dalam <span> dan bersihkan tanda kurung
                    const country = $entry('span').text().replace(/[()]/g, '').trim();

                    // Hapus <span> agar hanya tersisa teks tanggal
                    $entry('span').remove();

                    // ✅ Perbaikan Error 1: Gunakan $entry().text() untuk mengambil teks dari root
                    const date = $entry().text().trim();

                    // ✅ Perbaikan Error 2: Masukkan sebagai satu objek
