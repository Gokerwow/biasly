...
export async function SeedGroupsDetail() {
    ...
    await Promise.all(GroupsData.map(async (group) => {
                ...
                if (debutDetails) {
                    for (const detail of debutDetails) {
                        if (detail.country.includes('South Korea')) {
                            details.koreanDebutDate = detail.country
                        } else {
                            details.japanDebutDate = detail.country
                        }
                    }
                }
            }
        ...
        const genres = $('h3.pi-data-label:contains("Origin")')
            .next('pi-data-value')
            .text()

        details.genres = genres.split(',').map(genre => genre.trim())

        details.fandomName = $('h3.pi-data-label:contains("name")')
            .next('pi-data-value')
            .text()
            
        details.imageUrl = $('h3.pi-data-label:contains("name")')
            .next('pi-data-value')
            .text()
