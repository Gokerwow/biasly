            ...
            <div className="p-4">
                ...
                <div className="flex flex-wrap gap-2 mb-3 min-h-7">
                    {genres && genres.length > 0 ? (
                        // Jika ADA isinya, jalankan .map seperti biasa
                        genres.map((genre, index) => {
                            const color = genreColors[index % genreColors.length];
                            return (
                                <span key={index} className={`text-xs ${color.bg} ${color.text} px-2 py-1 rounded-full font-medium`}>
                                    {genre}
                                </span>
                            );
                        })
                    ) : (
                        // Jika KOSONG, tampilkan satu buah span placeholder ini
                        <span className="text-xs text-gray-400 px-2 py-1 italic">
                            No Genressssssssssssssss
                        </span>
                    )}
                </div>
