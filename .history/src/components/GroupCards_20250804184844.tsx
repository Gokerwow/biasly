...
export default function GroupCards({ name, genres, imageUrl }: GroupCardsProps) {
    const genreColors = [
    ...
    const validGenres = genres && genres.filter(genre => genre);

    return (
        <div className="bg-white rounded-xl overflow-hidden shadow-md cursor-pointer hover:shadow-lg hover:-translate-y-4 transition-all duration-300">
            <div className="relative h-60 overflow-hidden group">
                ...
                <div className="absolute bottom-0 left-0 p-4">
                    ...
                    <h2 className="text-purple-200 text-sm drop-shadow-md">JYP </h2>
                </div>
            ...
            <div className="p-4">
                ...
                <div className="flex flex-wrap gap-2 mb-3">
                    {validGenres && validGenres.length > 0 ?  (validGenres.map((genre, index) => {
                        const color = genreColors[index % genreColors.length]
                        return <span key={index} className={`text-xs ${color.bg} ${color.text} px-2 py-1 rounded-full font-medium`}>{genre ?? 'no genre'}</span>
                    })) :
                    <span className="text-xs text-gray-400 px-2 py-1 italic">
                        No Genre
                    </span>
                    }
                </div>
                ...
                <Link href="#" className="w-full bg-purple-600 hover:bg-purple-700 text-white py-2 rounded-lg text-sm font-semibold transition-colors duration-300 flex items-center justify-center gap-2 cursor-pointer">
                    View Profile
