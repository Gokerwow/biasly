...
export default function IdolsCard({ name, groups }: IdolsCardProps) {
    return (
        <div className="bg-white rounded-xl overflow-hidden shadow-md hover:shadow-card-hover hover:-translate-y-5 transition-all duration-200 hover:shadow-lg">
            <div className="relative group overflow-hidden">
                <div className="w-full h-60">
                    <Image src="/assets/images/1080full-yuna-(itzy).jpg" alt="Jisoo" fill className="object-cover group-hover:scale-110 transition-all duration-200" />
                </div>
                <div className="absolute inset-0 bg-[linear-gradient(0deg,rgba(0,0,0,0.4)_0%,rgba(0,0,0,0)_50%)]"></div>
                ...
                {/* {groups.map((group) =>
                    <div key={group.id} className="absolute bottom-3 left-3 bg-black/50 px-3 py-1 rounded-full text-white text-xs font-medium">
                        {group.name}
                    </div>
                )} */}

                <div key={groups.id} className="absolute bottom-3 left-3 bg-black/50 px-3 py-1 rounded-full text-white text-xs font-medium">
                    {groups.name}
                </div>
