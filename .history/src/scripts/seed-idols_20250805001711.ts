...
export async function seedDatabase() {
    try {
        ...
        const idolsDetailPromises = idolsData.map(async (idol) => {
            return limit(async () => {
                try {
                    const url = `https://kpop.fandom.com/api.php?action=parse&page=${idol.name}&prop=text&format=json&origin=*`
                    ...
                    if (fullFormatMatch) {
                        ...
                        console.log(`Tidak ditemukan nama stage dalam bahasa Korea atau Jepang untuk idol ${idol.name}`)
                    }
                    ...
                    if (birthDate) {
                        try {
                            ...
                            console.log(`Gagal parsing birth date untuk ${idol.name}: ${birthDate}`, error)
                    ...
                    const getSocial = (link: string) => {
                        try {
                            ...
                            console.log(`Link Sosmed untuk idol ${idol.name} bukan link valid ${error}`)
                            return {
                                platform: null,
                                url: null
                            }
                    ...
                    if (soloDebutText) {
                        try {
                            ...
                            console.log(`Gagal parsing solo debut untuk ${idol.name}: ${soloDebutText}`, error)
                        ...
                        const groupNamePromises = groupNameHtml.map(async (item) => {
                            ...
                            try {
                                ...
                                if (groupError && groupError.code !== 'PGRST116') {
                                    ...
                                    console.error(`Gagal mengambil data grup ${name} untuk idol ${idol.name}:`, groupError.message);
                                    return null;
                                }
                                ...
                                if (insertError) {
                                    console.error(`Gagal memasukkan grup ${name} untuk idol ${idol.name}:`, insertError.message);
                                    return null;
                                }
                    ...
                    console.error(`Gagal mengambil detail idol ${idol.name}:`, error);
                    return null; // Return null instead of throwing error
                }
            });
        })
        ...
        console.log(`Proses seeding selesai! ${idolDetails.length} idols berhasil diambil.`);
