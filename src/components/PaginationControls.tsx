...
export function PaginationControls({ totalPages, currentPage, page }: PaginationControlsProps ) {
    ...
    return (
        <div className="mt-12 flex justify-center">
            <nav className="flex items-center space-x-1">
                <Link href={`/${page}?page=${currentPage - 1}`} aria-label="previous pagination" className={`p-2 rounded-full bg-purple-600 text-white hover:bg-purple-700 cursor-pointer hover:scale-110 transition-all duration-200 ${currentPage === 1 ? 'pointer-events-none opacity-50' : ''}`}>
                ...
                <Link href={`/${page}?page=${currentPage + 1}`} aria-label="next pagination" className="p-2 rounded-full bg-purple-600 text-white hover:bg-purple-700 cursor-pointer hover:scale-110 transition-all duration-200">
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
